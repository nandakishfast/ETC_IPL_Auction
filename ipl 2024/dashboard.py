...
column_names = ['Player', 'Nationality', 'Type', 'Price']
...
app.layout = html.Div(style={'backgroundColor': 'black', 'color': 'white'}, children=[
    html.H1("ETC IPL Auction 2024", style={'text-align': 'center'}),
...
def update_output(button1_clicks, button2_clicks, player, team, price):
    ...
    team_list = ['RCB','SRH','CSK','KKR','MI','GT','RR','LSG']

    for team_name in team_list:
        cur.execute('SELECT p.name,p.country,p.type,b.price FROM players AS p INNER JOIN buys AS b ON p.id = b.player_id WHERE b.team = ?;',(team_name,))
        data = cur.fetchall()
        print(team_name, '-----------------')
        print(data)
        formatted_data = [dict(zip(['Player', 'Nationality', 'Type', 'Price'], row)) for row in data]
        ...
        cur.execute('SELECT count(p.country) FROM players AS p INNER JOIN buys AS b ON p.id = b.player_id WHERE b.team = ? AND p.country="Foreign";',(team_name,))
        ...
        cur.execute('SELECT count(p.type) FROM players AS p INNER JOIN buys AS b ON p.id = b.player_id WHERE b.team = ? AND p.type="WK";',(team_name,))
        wk = cur.fetchone()[0]
        ...
        info += "    Batsman   (min 3) : " + str(batsman) + '\n'
        info += " All Rounders (min 1) : " + str(all_round) + '\n'
